...
public class ProductListManager
{
	 ...
	 */
	public void RemoveItemFromList(String input, List<String> cartList)
	{
		for(int i = 0; i < productStringList.size(); i++)
		{
			...
			{
				cartList.add(productStringList.get(i));
	...
	/**
	 * @param newCartList Sets the productStringList to the input
	 */
	...
	/**
	 * Removes a Product from productList
	...
	/**
	 * @param newCartProducts Sets the productList to the input
	 */
	public void SetProducts(List<Product> newCartProducts) {productList = newCartProducts;}
	
	/**
	 * Takes a string list and chooses random points in that list to add to the product list
	 * @param list single list containing the .txt contents 
	 * @param count amount of times to loop through the .txt list
	 */
	public void PopulateListFromStringList(List<String> list, int count) 
	{
		Random random = new Random();
		
		for(int i = 0; i < count; i++) 
		{
			this.AddItemToList(list.get(random.nextInt(list.size())));
		}
	}
