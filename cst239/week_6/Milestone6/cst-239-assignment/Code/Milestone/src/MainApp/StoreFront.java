...
public class StoreFront
{
	 ...
	 */
	private static void ProductsMain()
	{
	...
	private static void AdminMain() throws IOException 
	{
		//u adds new products
		//r saves current products to list
		
		System.out.println("\n\nEnter Command: ");
		String command = mainScanner.next();
		switch(command.toLowerCase())
		{
		case "u": 
			System.out.println("\n\nHow many products to add? ");
			int productsToAdd = mainScanner.nextInt();
			if(productsToAdd > 0) 
			{
				AddNewProducts(productsToAdd); 
				System.out.println(productsToAdd + "products added!");
			}
			break;
		case "r":
			System.out.println("Saving products to file...");
			SaveCurrentProductsToFile();
		break;
		}
		mainInput = 0;
	}
	 ...
	 */
	private static boolean ProductMenu(int input) 
	{
	 ...
	 */
	private static Product ChooseProducts(int input) 
	{
	 ...
	 */
	private static void ProductToCart(Product itemToBeInCart) 
	{
	 ...
	 */
	private static boolean RemoveProduct(int input) 
	{
		switch (input)
		{
		case 1: 
	 ...
	 * @param input User input from scanner
	 */
	private static void PurchaseProductsInCart(int input) 
	{		
		boolean amountGreaterThanOne = cart.GetItemsInCart() > 1;
		switch (input)
		{
	...
	private static void AddNewProducts(int amount) throws IOException 
	{
		List<String> singleProductList = FileManager.FileToStringList("Input.txt");
		
		productListManager.PopulateListFromStringList(singleProductList, amount);
	}
	
	private static void SaveCurrentProductsToFile() throws FileNotFoundException 
	{
		PrintWriter out = new PrintWriter("OutPut.txt");
		
		for (String string : productListManager.GetList())
		{
			out.println(string);
		}
		out.close();
	 ...
	 */
	private static void DisplayMenu(boolean cartMenu, List<String> stringList, List<Product>  productList, String type)
	{
	 ...
	 * @param type For Products only, it is the type of category that the User is entering
	 */
	private static void UpdateTopString(boolean cartR, String type) 
	{
		if(cartR) 
		{
	...
	/**
	 * Resets how the User wants to sort
	 */
