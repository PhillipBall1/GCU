...
public class Product implements Comparable<Product>
{
	...
	{
		if(StoreFront.ascendingNameSort)
		{
			return this.name.compareTo(product.GetName());
		}
		if(StoreFront.descendingNameSort)
		{
			return product.GetName().compareTo(this.name);
		}
		if(StoreFront.ascendingPriceSort)
		{
			if(this.price > product.GetPrice()) return 1;
			if(this.price < product.GetPrice()) return -1;
			if(this.price == product.GetPrice()) return 0;
		}
		if(StoreFront.descendingPriceSort)
		{
			if(this.price > product.GetPrice()) return -1;
			if(this.price < product.GetPrice()) return 1;
			if(this.price == product.GetPrice()) return 0;
